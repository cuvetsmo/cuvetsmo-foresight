...
/**
 * Local fallback seed — mirror of what's in Supabase `public.foresight_markets`
 * (and `cuvetsmo-foresight-mcp/src/data/markets.ts`).
 *
 * Used when:
 *   1. NEXT_PUBLIC_SUPABASE_URL / ANON_KEY env vars are unset (e.g., fresh
 *      clones, preview branches without secrets).
 *   2. The Supabase query errors out — we degrade gracefully instead of
 *      blanking the landing.
 *
 * To keep this in sync after the schema is the source of truth: run
 * `select * from public.foresight_markets order by id` and update this array.
 */
